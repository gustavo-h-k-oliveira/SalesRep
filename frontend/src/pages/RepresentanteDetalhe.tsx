    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 text-sm text-slate-600 sm:justify-end">
            <div className="flex items-center gap-1.5 rounded-xl border border-slate-100 bg-slate-50/50 px-3.5 py-2">
              ...
              <span>
                {rep.estadoNome ? (
                  <>Estado: <strong>{rep.estadoNome} ({rep.estadoUf})</strong> • Região <strong>{rep.regiaoNome}</strong></>
                ) : (
                  <>Região: <strong>{rep.regiaoNome}</strong></>
                )}
              </span>
            </div>
            <div className="flex items-center gap-1.5 rounded-xl border border-slate-100 bg-slate-50/50 px-3.5 py-2">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 gap-3">
          ...
          {/* Botão de ação rápida na aba ativa */}
          {gestor && (
            <div className="pb-3 sm:pb-2">
              {activeTab === 'clientes' && (
                <Link
                  to={`/representantes/${rep.id}/cadastro?tipo=cliente`}
                  className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200/80 px-3 py-1.5 text-xs font-semibold transition-all shrink-0"
                >
                  <UserPlusIcon className="h-3.5 w-3.5" weight="bold" />
                  <span>Novo Cliente</span>
                </Link>
              )}
              {activeTab === 'pedidos' && (
                <Link
                  to={`/representantes/${rep.id}/cadastro?tipo=pedido`}
                  className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200/80 px-3 py-1.5 text-xs font-semibold transition-all shrink-0"
                >
                  <ShoppingCartIcon className="h-3.5 w-3.5" weight="bold" />
          ...
          {activeTab === 'clientes' && (
            <div>
              {clientes.length > 0 ? (
                ...
                <div className="rounded-2xl border border-slate-200 border-dashed bg-slate-50/50 p-8 text-center flex flex-col items-center justify-center">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100 text-slate-400 mb-3">
                    <UsersIcon className="h-6 w-6" />
                  </div>
                  <p className="text-sm font-semibold text-slate-800">Este representante não possui clientes cadastrados.</p>
                  <p className="mt-1 text-xs text-slate-500 max-w-sm">
                    Adicione o primeiro cliente à carteira de atendimento comercial deste representante.
                  </p>
                  {gestor && (
                    <Link
                      to={`/representantes/${rep.id}/cadastro?tipo=cliente`}
                      className="mt-4 inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-700 shadow-xs active:scale-95 transition-all"
                    >
            ...
            <div>
              {pedidos.length > 0 ? (
                ...
                <div className="rounded-2xl border border-slate-200 border-dashed bg-slate-50/50 p-8 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm font-semibold text-slate-800">Nenhum pedido registrado para este representante.</p>
                  <p className="mt-1 text-xs text-slate-500 max-w-sm">
                    Registre uma nova emissão de pedido comercial para começar a movimentar a carteira.
                  </p>
                  {gestor && (
                    <Link
                      to={`/representantes/${rep.id}/cadastro?tipo=pedido`}
